 ...
 *   events.connect();
 *
 * Text cleaning is server-side: pass clean:true (sends extra_body.clean:true)
 * or call cleanText() to get the cleaned text back. cleanMarkdown() is the
 * exported local copy for offline/preview use.
 *
 * Contents:
 *   1. Errors
...
//
//  server/markdown-clean.js re-exports these; the server runs them when a
//  request passes extra_body.clean. Clients pass clean:true (server cleans)
//  or call cleanText() to get the cleaned text back (alignment use case).
//  This local copy exists for offline/preview use. NO imports — must stay
//  dependency-free.
//
...
export class NSpeechClient {
  ...
  /**
   * Clean text server-side. Returns the exact text the speech endpoint would
   * speak — use this when you need the cleaned text (e.g. text/audio
   * alignment), then send it to speech() without clean.
   *
   * @param {string} text — raw text/markdown
   * @param {string} [mode='regex'] — 'regex' | 'llm'
   * @param {AbortSignal} [signal]
   * @returns {Promise<{text: string, chars_before: number, chars_after: number}>}
   */
  async cleanText(text, mode = 'regex', signal) {
    if (typeof text !== 'string' || !text.length) throw new Error('cleanText: text required');
    const reqId = this._requestId();
    this._log(`[${reqId}] cleanText: mode=${mode} chars=${text.length}`);

    return _retry(async () => {
      const res = await this._fetch('/v1/text/clean', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
