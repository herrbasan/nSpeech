...
- **Deadlock trap (hit, then fixed)** — the resident warm must read the worker *directly*. Routing it through `refresh()` re-enters `_fetchVoices`, which awaits the pending resident-warm promise when no catalog is cached yet: the promise awaits itself, never settles, and Kokoro's built-ins are silently never captured. Symptom to recognise: `/v1/admin/cache` shows `builtins: 0` for Kokoro and the log has no `resident engine warmed` entry. Verified fixed by probing the warm path directly — Kokoro lands 60 voices / 54 built-ins.
